...
export function FormTicket({ customer }: FormTicketProps){
  ...
  return(
    ...
    >
      ...
      <label className='mb-1 font-medium
      text-lg'>Descreva o problema</label>

      <textarea 
        className='w-full border-2 rounded-md h-24
        resize-none px-2'
        placeholder='Descreva o seu problema...'
        id="description"
        {...register("description")}
      ></textarea>
      {errors.description?.message && <p className='text-red-500 
      mt-1 mb-4 my-1'>{errors.description?.message}</p>}
    
      <button type='submit' className='bg-blue-500 rounded-sm
      w-full h-11 px-2 text-white font-bold'>
        Cadastrar
